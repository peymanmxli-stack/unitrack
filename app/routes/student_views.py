...
def build_student_access_qr_id_code(student):
    ...
    raw_text = f"{student.id}|{student.username}|{student.first_name}|{student.last_name}"
...
def find_student_by_access_qr_id_code(qr_id_code):
    """
    Resolve a student from the short manual QR ID code.
    """
    ...
    for student in students:
        if build_student_access_qr_id_code(student) == qr_id_code:
            return student
...
def process_access_scan_qr_text(qr_text):
    ...
    if parsed_payload:
        ...
        if student.username != qr_username:
            return {
                "success": False,
                "message": "QR validation failed. Student data mismatch."
            }, 400

    else:
        student = find_student_by_access_qr_id_code(qr_text)
    ...
    open_log = get_open_access_log_for_student(student.id)
    ...
    try:
        if open_log:
            ...
            return {
                ...
                "qr_id_code": build_student_access_qr_id_code(student),
...
def configuration_update():
    ...
    if password_change_requested:
        if not current_password:
            return _redirect_configuration_error("La contraseña actual es obligatoria para cambiar la contraseña.")

        if not verify_password(current_user.password_hash, current_password):
            return _redirect_configuration_error("La contraseña actual es incorrecta.")

        if not new_password:
            return _redirect_configuration_error("La nueva contraseña es obligatoria.")

        if not confirm_password:
            return _redirect_configuration_error("Debes confirmar la nueva contraseña.")

        if new_password != confirm_password:
            return _redirect_configuration_error("La nueva contraseña y la confirmación no coinciden.")

        if new_password == current_password:
            return _redirect_configuration_error("La nueva contraseña debe ser diferente a la contraseña actual.")
    ...
    try:
        ...
        if password_change_requested:
            return _redirect_configuration_message(
                "Configuración y contraseña actualizadas correctamente."
            )
...
def configuration_update_photo():
    ...
    try:
        ...
        current_user.photo_path = f"uploads/students/{final_filename}"
        db.session.commit()
